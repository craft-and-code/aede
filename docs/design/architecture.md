...
`release.yml` fires on a tag matching `v*` and builds three archives — macOS on Apple Silicon and Intel, and Linux — each holding the binary, the licence and the manual's front page, with a `.sha256` beside it. Windows remains absent pending native validation of the path correction; [Paths](paths.md) describes the native-spelling contract and the formerly ignored tests now enabled in Windows CI. Three decisions are worth keeping:
...
503 tests: binary parsers (including truncated files and forged signatures), name normalization, graph construction, persistence round-trip, statistics, diagnostics, table alignment, argument parsing, an end-to-end test that runs the binary, and a check that no link in this manual leads nowhere. The conversion tests skip themselves, loudly, when ffmpeg is not installed.
